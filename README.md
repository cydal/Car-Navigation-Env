...
red light in traffic: oriented detection boxes on the vehicles the front/left/right/
rear cameras can see, and LIDAR range shown as a pulsing wave band breathing out
from the car and back, clipped per direction to the actual measured distance
(see [Live browser viewer](#live-browser-viewer) for why). Every panel reads
real server-computed state — nothing here is a client-side approximation of
what the car senses. Screenshotted headlessly with `tools/viewer_shot.py`; the
task/agent-view figures further down are regenerated by
...
client-side approximation), LIDAR range as a pulsing wave band around the car
and a per-sector radar readout, blind-spot chips, the reward and its
components, a minimap and a 60 s history of which speed
...
in world-frame metres. The viewer draws each one as a trail of small dots
ahead of the car, shrinking and fading toward the end of the horizon --
dots rather than a line so it stays legible viewed from any angle (a thin
line nearly end-on, the common case for a mostly-forward path, foreshortens
toward invisibility) and reads as visually distinct from the pulsing LIDAR
wave around the car -- and shows an `imagining ×N · H steps` chip next to the
driver's name. Both only appear for an agent that actually provides this;
every other agent's `sensors` overlay is unaffected. Nothing here is RL- or
Dreamer-specific: any agent that can produce a sequence of predicted `(x, y)`
points qualifies, and most agents simply never set the key. See
