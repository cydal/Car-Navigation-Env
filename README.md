...
rear cameras can see, and LIDAR range shown as a ring of proximity dots around the
car rather than a fan of rays back to it (see [Live browser
viewer](#live-browser-viewer) for why). Every panel reads real server-computed
state — nothing here is a client-side approximation of what the car senses.
Screenshotted headlessly with `tools/viewer_shot.py`; the task/agent-view figures
further down are regenerated by
...
client-side approximation), LIDAR range as a ring of proximity dots around the
car and a per-sector radar readout, blind-spot chips, the reward and its
...
toward invisibility) and reads as visually distinct from the LIDAR dots
around the car -- and shows an `imagining ×N · H steps` chip next to the
